...
function prefix_load_plugin_textdomain() {
    load_plugin_textdomain( 'codeby-core', false, '/codeby-core/languages/' );
}
...
function prefix_settings_link( $links ) {
	return array_merge(
		array(
			'settings' => '<a href="' . admin_url( 'options-general.php?page=codeby-core' ) . '">' . __( 'Settings', 'codeby-core' ) . '</a>'
		),
		$links
	);
}
add_filter( 'plugin_action_links_' . PREFIX_CODEBY_CORE . '/codeby-core.php', 'prefix_settings_link' );
...
function prefix_plugin_row_meta( $links, $file ) {
	if ( strpos( $file, 'codeby-core.php' ) !== false ) {
		$new_links = array(
...
//				'kuttappi' 	=> '<a href="http://kuttappi.com/" target="_blank">My Travelogue</a>',
				'hireme' 	=> '<a href="https://codeby.com" target="_blank">Hire Me For A Project</a>',
				);
...
function prefix_footer_text($default) {
	...
	if ( $screen->id !== "settings_page_codeby-core" ) {
		return $default;
	}
	
    $prefix_footer_text = sprintf( __( 'If you like this plugin, please <a href="%s" target="_blank">make a donation</a> or leave me a <a href="%s" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a> rating to support continued development. Thanks a bunch!', 'codeby-core' ),
								'http://millionclues.com/donate/',
								'https://wordpress.org/support/plugin/codeby-core/reviews/?rate=5#new-post'
						);
